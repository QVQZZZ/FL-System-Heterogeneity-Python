...
def fjord(clients, clients_per_round, total_epochs, local_epochs):
    ...
    for total_epoch in range(total_epochs):
        ...
        for client in clients_selected:
            ...
            for i in range(len(client_models)):
                client_model = get_parameters_from_server(client_models[i], global_model)
                client_models[i] = client_model

            teacher = max(client_models, key=lambda client_model: client_model.width_factor)
            teacher_idx = client_models.index(teacher)
            students = client_models[:teacher_idx] + client_models[teacher_idx+1:]
...
if __name__ == '__main__':
    ...
    selected_rate = 0.1
    final_global_model = fjord(clients=clients, clients_per_round=int(selected_rate * num_clients), total_epochs=50,
